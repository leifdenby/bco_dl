...
- available data: http://bcoweb.mpimet.mpg.de/systems/data_availability/DeviceAvailability.html
"""
import io
import numpy as np
import bz2
...
def gen_queries(p_format, t_start, t_end):
    """
    For a path format which includes datetime placeholders generate the queries which
    will fetch all files in the time range between `t_start` and `t_end`
    """
    if '%d' in p_format:
...
def get_dataset(c_ftp, remote_path, in_memory=False):
    """
